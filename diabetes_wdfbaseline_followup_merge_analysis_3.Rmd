...
#library we need
libs_diabetes <- c("googledrive" ,"tidyverse", "haven", "janitor", "knitr",
                            "lubridate", "plotly", "gtsummary", "flextable", "sjlabelled", "rstatix",
                   "lme4", "lmerTest")
...
```{r}
  ...
  wdfbaseline_followup_final_analysis%>%
    dplyr::select(wdf_q1_9, wdf_q1_4_age, wdf_q5_8a, wdf_q5_8b, wdf_q5_10, wdf_q5_1_2_3a, wdf_q5_1_2_3b, wdf_q5_6_7, 
    wdf_q5_5_6_7_bmi, wdf_q5_8a_b_waist_hip_ratio )%>%
  #sjlabelled:::label_to_colnames() %>% #variable labels to column names.
  group_by(wdf_q1_9)%>%
  shapiro_test(wdf_q1_4_age, wdf_q5_8a, wdf_q5_8b, wdf_q5_10, wdf_q5_1_2_3a, wdf_q5_1_2_3b, wdf_q5_6_7, 
    wdf_q5_5_6_7_bmi, wdf_q5_8a_b_waist_hip_ratio)%>%
...
diabetes_mixed_model <- wdfbaseline_followup_final_analysis%>%
  ...
  mutate(clinical_parameters = as_factor(clinical_parameters))%>%
  nest_by(clinical_parameters)%>%
  mutate(lmm_model = list(lmer(value ~ wdf_q1_5 + wdf_q1_4_age_group + 
        ...
        table_out = list(tbl_regression(lmm_model, exponentiate = FALSE, #broom.mixed package required
    pvalue_fun = ~style_pvalue(.x, digits = 3))%>%
  #add_global_p()%>% # add global p-value for categorical variables
  bold_p(t= 0.05) %>% # bold p-values under a given threshold (default 0.05)
  bold_labels() %>%
  italicize_levels()%>% 
  modify_header(label = "**Linear Mixed Model Regression**")%>% # update the column header
...
tbl_merge(diabetes_mixed_model$table_out,
    tab_spanner = c("**Systolic Blood pressure**", "**Diastolic Blood pressure**", "**Blood glucose (mmol/L)**", "**BMI (kg/m2)**",
                    "**Weight in kgs**", "**Waist circumference**", "**Waist-to-hip ratio**", "**Hip circumference**"))
...
for (i in 1:length(diabetes_mixed_model$clinical_parameters)){
  ...
  diabetes_mixed_model_metrics[i] <- print(
    kable(
    list(broom::glance(diabetes_mixed_model$lmm_model[[i]])),
