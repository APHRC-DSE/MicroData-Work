...
```{r googledrive downloading sero data, results="hide", include=FALSE}
 ...
 drive_auth(#email = "guest360@aphrc.org"
  ...
  sero_file_id <- drive_find(shared_drive = c(shared_drive_find(pattern = "Data Science Programs")$id),
           q = c("name = 'sero_results_data.dta'")) #search for a specific set of shared drives, use the query string q
...
scan_report_id <- drive_find(shared_drive = c(shared_drive_find(pattern = "Data Science Programs")$id),
           q = c("name = 'ScanReport_sero.xlsx'")) #search for a specific set of shared drives, use the query string q
...
```{r sero final raw data, results="hide", include=FALSE}

sero_final_raw <- sero_final%>%
               mutate(
...
ses_df <- sero_final_raw%>%
  dplyr::select(studyid, q17_electricity:q17_clock, q17_floor_type, q17_wall_type, q17_roof_type, q17_fuel_type,
                q17_toilet_type)%>%
...
#The first principal component explains the largest proportion of the total variance and it is used as the wealth index to represent #the household's wealth
...
sero_final_clean <- sero_final_raw%>%
                    left_join(ses_df%>%   
                       mutate(ses_scores = ses_scores)%>%
                         dplyr::select(studyid, ses_scores), by = c("studyid" = "studyid"))%>%
                      mutate(ses_index = cut(ses_scores, breaks = 5, 
                                             labels = c("Quintile1", "Quintile2", "Quintile3", "Quintile4", "Quintile5")))%>%
                       labelled::set_variable_labels(#labeling created variables
     ses_scores = "Socio-economic scores",
...
readr::write_csv(sero_final_clean%>%
                   dplyr::select(-c(q7_age_new, age_group_new, ses_scores, ses_index)),"sero_results_data_clean.csv", na="")

haven::write_dta(sero_final_clean%>%
                   dplyr::select(-c(q7_age_new, age_group_new, ses_scores, ses_index)),"sero_results_data_clean.dta")
...
```{r data description}
  ...
  tbl_summary(sero_final_analysis%>%dplyr::select(consent, q5_status), 
                      type = list(
...
```{r all variable descriptives}
 ...
 tbl_summary(sero_final_analysis%>%dplyr::select(-c(consent, q5_status)), 
                      type = list(
                        all_dichotomous() ~ "categorical"
...
```{r ses scores ggplot histogram, fig.width=10, fig.height=6}

sero_final_clean%>%
  dplyr::select(ses_scores)%>%
  ggplot(aes(x = ses_scores))+ 
  geom_histogram(bins = 50, color = "black", fill = "gray") +
  geom_vline(aes(xintercept = mean(ses_scores, na.rm=TRUE)), 
...
```{r ses scores ggplot histogram and density, fig.width=10, fig.height=6}
