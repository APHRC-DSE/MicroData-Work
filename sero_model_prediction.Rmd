...
sero_logistic_drop_missing <- sero_final_analysis%>%
  mutate(across(c(spikepos, replace, q11_fever:q11_lost_smell, q13_hhm_no_symptoms, q13_hhmember_sick,
                  q17_electricity:q17_none, q17_floor_type:q17_toilet_type, q12_hospitalized), ~ fct_rev(.x)))%>%
  dplyr::select(-c(sample_type, q11_pain, q11_no_symptoms, q23_sampling, q7_age_new, 
                   #age_group_new,
...
rf_model <- randomForest(spikepos ~., data = train_sero, importance = FALSE, mtry = round(ncol(train_sero)/3,0))
rf_model
#summary(rf_model)
...
#Fitting with test data
test_predictions$rf_prediction <- predict(rf_model, test_sero, type = "response" #“prob”, “response”, “vote”
                                          )
...
roc_rf_model <- performance::performance_roc(x= c(as.numeric(test_sero$spikepos)-1),
                              predictions = c(as.numeric(predict(rf_model, test_sero, type = "response"))-1) )
#package see required to plot ROC curves
#plot(roc_rf_model)
...
varfreq_df <- train_importance%>% 
  ...
  mutate(pos_new=ifelse(pos<=top_n, pos, top_n+1),
         new_terms=fct_reorder(variables, pos)
	)%>%
  group_by(new_terms, pos_new)%>%
  count()%>% 
  droplevels()

ggplot(varfreq_df, aes(x=pos_new, y=fct_reorder(new_terms, -pos_new), fill=n))+
