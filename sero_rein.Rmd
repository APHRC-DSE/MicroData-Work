...
scan_report_field_overview <- readxl::read_xlsx("ScanReport_sero.xlsx",sheet = "Field Overview")%>%
  drop_na(Table)
...
sero_final_analysis <- sero_final_clean%>%
  dplyr::select( #socio-demographics
    ...
    q11_no_symptoms,q11_fever:q11_other_symptom, q11_muscular_pain:q11_joint_pain,
                q11_muscular_pain_overall:q11_joint_pain_overall, q12_medical_attention, q12_miss_work, q12_hospitalized,
                #health symptoms of household members
   q13_hhmember_sick, q15_hhm_died, q13_hhm_no_symptoms, q13_hhm_fever:q13_hhm_other_symptom, 
                    q13_hhm_muscular_pain:q13_hhm_joint_pain, q13_hhm_muscular_pain_overall:q13_hhm_joint_pain_overall,
   ...
   q10_covid_contact, q16_own_risk, q16_info_edu, q16_hand_wash, q16_face_masks, q16_sanitizers,
                                covid_vaccine, q19_lost_job, q20_healthcare_access,
                #socio-economic status
   q17_electricity:q17_none, q17_floor_type, q17_wall_type, q17_roof_type, q17_fuel_type, q17_toilet_type,
...
```{r data description}
  ...
  tbl_summary(sero_final_clean%>%dplyr::select(consent, replace, q5_status), 
                      type = list(
...
For continous data, Wilcoxon rank sum test was used to examine the comparison of outcome (sero positive), location, gender, school level and age group variables with age. 
...
```{r all variables by education level}
  ...
  add_p(pvalue_fun = ~style_pvalue(.x, digits = 3)
        ,test.args = c(q6_religion, q10_covid_contact, q16_info_edu, q16_hand_wash, q16_face_masks, q19_lost_job,
                       ...
                       q18_outmigrant_member, q18_inmigrant_member, blood_group) ~ list(simulate.p.value=TRUE)) %>%
   bold_p(t= 0.05) %>% # bold p-values under a given threshold (default 0.05)
  #add_overall() %>%
  #add_difference() %>% #add column for difference between two group, confidence interval, and p-value
