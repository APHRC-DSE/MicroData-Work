...
#library we need
libs_supermarket_store <- c("googledrive" ,"tidyverse", "haven", "janitor", "knitr",
                            "lubridate", "plotly", "gtsummary", "flextable", "fastDummies", "labelled")

#install missing libraries

inatalled_libs_supermarket_store <- libs_supermarket_store  %in% rownames(installed.packages())
if (any(inatalled_libs_supermarket_store==F)) {
  install.packages(libs_supermarket_store[!inatalled_libs_supermarket_store])
}

#load libraries

invisible(lapply(libs_supermarket_store, library, character.only=T))
...
```{r reading supermarket data locally, results="hide", include=FALSE}
  ...
  labelled::set_variable_labels( #labeling unlabeled variables and relabling for final data dictionary
    ...
    q4p2b1_297 = "NAN.",
    q4p2b1_298 = "Similac.",
    q4p2b1_299 = "Nutristart",
    q4p2b1_300 = "others (specify)",
    ...
    q7p2a = "Q7.2: Food/ drink product group sold",
    q7p2b = "Q7.2: Other name specify sold",
    q7p3 = "Q7.3: Food / drink product name sold",
    q7p5 = "Q7.5: Types of foods sold in the outlet: Cooked, Uncooked or both",
    q7p6 = "Q7.6: Food product description and Brand Name sold",
...
# Dummy code location, adverts at location, size of advertisement, Type of advertisement, Number of products in advertisement,
# Content of advert ( pictorial , text, combined), Food/drink product group sold,  Types of foods sold in the outlet cooked, uncooked or both,
#shelves number where food products are placed,
supermarket_store_final_set1 <- supermarket_store_final%>%
  dplyr::select(key,location, advert, q3p5a, q3p7, q3p8, q3p10, q7p2a, q7p5, shelves)%>%
  ...
  janitor::clean_names()%>%
  dplyr::select(-c(advert_no))%>% #unselect variable
  labelled::set_variable_labels( #labeling new variables
    ...
    q7p2a_whole_grain_cereals = "Food/drink product group sold - Whole grain cereals",
    q7p2a_refined_cereals = "Food/drink product group sold - Refined cereals",
    q7p2a_fermented_grain_products = "Food/drink product group sold - Fermented grain products",
    q7p2a_milk_and_milk_products = "Food/drink product group sold - Milk and milk products",
    q7p2a_fresh_juices = "Food/drink product group sold - Fresh juices",
    q7p2a_vitamin_a_rich_fruits = "Food/drink product group sold - Vitamin A rich fruits",
    q7p2a_other_fruits = "Food/drink product group sold - Other fruits",
    q7p2a_nuts_and_seeds = "Food/drink product group sold - Nuts and seeds",
    q7p2a_roots_tubers_plantain_and_potatoes = "Food/drink product group sold - Roots, tubers, plantain and potatoes",
    q7p2a_vitamin_a_rich_vegetables = "Food/drink product group sold - Vitamin A rich vegetables",
    q7p2a_green_leafy_vegetables = "Food/drink product group sold - Green leafy vegetables",
    q7p2a_other_vegetables = "Food/drink product group sold - Other vegetables",
    q7p2a_pulses_beans_peas_lentils = "Food/drink product group sold - Pulses (beans, peas, lentils)",
    q7p2a_soups = "Food/drink product group sold - Soups",
    q7p2a_egg = "Food/drink product group sold - Egg",
    q7p2a_red_meat = "Food/drink product group sold - Red meat",
    q7p2a_organ_meat = "Food/drink product group sold - Organ meat",
    q7p2a_poultry = "Food/drink product group sold - Poultry",
    q7p2a_fish = "Food/drink product group sold - Fish",
    q7p2a_mixed_dishes_vegetarian_meaty = "Food/drink product group sold - Mixed dishes (vegetarian, meaty)",
    q7p2a_savoury_pies = "Food/drink product group sold - Savoury pies",
    q7p2a_processed_meat = "Food/drink product group sold - Processed meat",
    q7p2a_sugar_and_sweet_spreads = "Food/drink product group sold - Sugar and sweet spreads",
    q7p2a_cakes_and_sweets = "Food/drink product group sold - Cakes and sweets",
    q7p2a_oils = "Food/drink product group sold - Oils",
    q7p2a_snacks_savoury_or_sweet = "Food/drink product group sold - Snacks (savoury or sweet)",
    q7p2a_spreading_fats = "Food/drink product group sold - Spreading fats",
    q7p2a_fats = "Food/drink product group sold - Fats",
    q7p2a_sodas_and_sweetened_beverages = "Food/drink product group sold - Sodas and sweetened beverages",
    q7p2a_tea_and_coffee = "Food/drink product group sold - Tea and coffee",
    q7p2a_alcoholic_beverages = "Food/drink product group sold - Alcoholic beverages",
    q7p2a_condiments_and_flavour_cubes = "Food/drink product group sold - Condiments and flavour cubes",
    q7p2a_water = "Food/drink product group sold - Water",
    q7p2a_infant_formula = "Food/drink product group sold - Infant formula",
    ...
    q7p5_not_with_cooked_uncooked_category = "Q7.5: Types of foods sold/advertised in the outlet - Not with Uncooked and Cooked category",
...
supermarket_store_final_set2 <- supermarket_store_final%>%
  ...
  mutate(n = 1:n())%>%
  pivot_wider(names_from = q4p1, values_from = q4p1, values_fn = ~1, values_fill = 0, #values_fn = length
              names_expand = TRUE, #Forces implicit(unused) factor levels to explicit. be represented in result
              names_prefix = "q4p1_")%>%
  janitor::clean_names()%>%
  dplyr::select(-c(q4p1_na, n))%>%
...
supermarket_store_final_set3 <- supermarket_store_final%>%
  dplyr::select(key, q4p2a, food_cat, food_cat5, InformasCat, Nova_sales, Prominence_Level)%>%
  fastDummies::dummy_cols(select_columns = c("q4p2a","food_cat", "food_cat5", "InformasCat", "Nova_sales", "Prominence_Level"),
             ignore_na = TRUE, #ignores any NA values in the column
             remove_selected_columns = TRUE #removes the columns used to generate the dummy columns.
             )%>%
  janitor::clean_names()%>%
  labelled::set_variable_labels( #labeling new variables
    q4p2a_whole_grain_cereals = "Food/drink product group advertised - Whole grain cereals",
    q4p2a_refined_cereals = "Food/drink product group advertised - Refined cereals",
    q4p2a_fermented_grain_products = "Food/drink product group advertised - Fermented grain products",
    q4p2a_milk_and_milk_products = "Food/drink product group advertised - Milk and milk products",
    q4p2a_fresh_juices = "Food/drink product group advertised - Fresh juices",
    q4p2a_vitamin_a_rich_fruits = "Food/drink product group advertised - Vitamin A rich fruits",
    q4p2a_other_fruits = "Food/drink product group advertised - Other fruits",
    q4p2a_nuts_and_seeds = "Food/drink product group advertised - Nuts and seeds",
    q4p2a_roots_tubers_plantain_and_potatoes = "Food/drink product group advertised - Roots, tubers, plantain and potatoes",
    q4p2a_vitamin_a_rich_vegetables = "Food/drink product group advertised - Vitamin A rich vegetables",
    q4p2a_green_leafy_vegetables = "Food/drink product group advertised - Green leafy vegetables",
    q4p2a_other_vegetables = "Food/drink product group advertised - Other vegetables",
    q4p2a_pulses_beans_peas_lentils = "Food/drink product group advertised - Pulses (beans, peas, lentils)",
    q4p2a_soups = "Food/drink product group advertised - Soups",
    q4p2a_egg = "Food/drink product group advertised - Egg",
    q4p2a_red_meat = "Food/drink product group advertised - Red meat",
    q4p2a_organ_meat = "Food/drink product group advertised - Organ meat",
    q4p2a_poultry = "Food/drink product group advertised - Poultry",
    q4p2a_fish = "Food/drink product group advertised - Fish",
    q4p2a_mixed_dishes_vegetarian_meaty = "Food/drink product group advertised - Mixed dishes (vegetarian, meaty)",
    q4p2a_savoury_pies = "Food/drink product group advertised - Savoury pies",
    q4p2a_processed_meat = "Food/drink product group advertised - Processed meat",
    q4p2a_sugar_and_sweet_spreads = "Food/drink product group advertised - Sugar and sweet spreads",
    q4p2a_cakes_and_sweets = "Food/drink product group advertised - Cakes and sweets",
    q4p2a_oils = "Food/drink product group advertised - Oils",
    q4p2a_snacks_savoury_or_sweet = "Food/drink product group advertised - Snacks (savoury or sweet)",
    q4p2a_spreading_fats = "Food/drink product group advertised - Spreading fats",
    q4p2a_fats = "Food/drink product group advertised - Fats",
    q4p2a_sodas_and_sweetened_beverages = "Food/drink product group advertised - Sodas and sweetened beverages",
    q4p2a_tea_and_coffee = "Food/drink product group advertised - Tea and coffee",
    q4p2a_alcoholic_beverages = "Food/drink product group advertised - Alcoholic beverages",
    q4p2a_condiments_and_flavour_cubes = "Food/drink product group advertised - Condiments and flavour cubes",
    q4p2a_water = "Food/drink product group advertised - Water",
    q4p2a_infant_formula = "Food/drink product group advertised - Infant formula",
...
supermarket_store_final_set1_2_3_labels <- as.data.frame(labelled::generate_dictionary(
  purrr::reduce(list(supermarket_store_final_set1%>%mutate(n = 1:n())%>%dplyr::select(-key),
                     supermarket_store_final_set2%>%mutate(n = 1:n())%>%dplyr::select(-key),
                     supermarket_store_final_set3%>%mutate(n = 1:n())%>%dplyr::select(-key) ),
                                         dplyr::left_join, by = c('n'))%>%dplyr::select(-n)
))%>%
  select(variable, label)%>%
  deframe()

#Final clean data set
supermarket_store_final_raw <- bind_cols(supermarket_store_final, supermarket_store_final_set1%>%dplyr::select(-key),
                                           supermarket_store_final_set2%>%dplyr::select(-key),
                                           supermarket_store_final_set3%>%dplyr::select(-key))
  
  
  supermarket_store_final_clean <- supermarket_store_final%>%
  dplyr::select(county:interviewdate, New_StoreLength1:id, locationexists, food, Supermarket_Sizea, SESa)%>%
  distinct(key, .keep_all = TRUE)%>%
    left_join(
  supermarket_store_final_raw%>%
    group_by(key)%>%
    summarise(across(c(location_entrance:advert_yes, q3p5a_small_a4_but_1_3m_x_1_9m:q3p10_combined, q5p1a:q5p1a_96, q5p2a:q5p2a_96,
                     q7p2a_whole_grain_cereals:q7p2a_infant_formula, q7p3_1:q7p3_300, q7p5_cooked:q7p5_both, 
                     q7p5_not_with_cooked_uncooked_category, shelves_shelf1_5:shelves_shelf_15ormore,
                     level_1:level_3, q4p1_4us:q4p1_asis, q4p2a_whole_grain_cereals:q4p2a_infant_formula, q4p2b1_1:q4p2b1_300,
  ...
  mutate(across(c(location_entrance:advert_yes, q3p5a_small_a4_but_1_3m_x_1_9m:q3p10_combined, q5p1a:q5p1a_96, q5p2a:q5p2a_96,
                     q7p2a_whole_grain_cereals:q7p2a_infant_formula, q7p3_1:q7p3_300, q7p5_cooked:q7p5_both, 
                     q7p5_not_with_cooked_uncooked_category, shelves_shelf1_5:shelves_shelf_15ormore,
                     level_1:level_3, q4p1_4us:q4p1_asis, q4p2a_whole_grain_cereals:q4p2a_infant_formula, q4p2b1_1:q4p2b1_300,
            food_cat_chocolate_and_sugar_confectionery_energy_bars_sweet_topping_and_desserts:food_cat_sauces_dips_other_seasonings_and_dressings,
            ...
            prominence_level_low_prominence:prominence_level_high_prominence), ~ifelse(.x == 0, "No",
                                 ifelse(.x > 0 , "Yes", .x))),
         across(c(location_entrance:advert_yes, 
                     q7p2a_whole_grain_cereals:q7p2a_infant_formula, q7p3_1:q7p3_300, q7p5_cooked:q7p5_both, 
                     q7p5_not_with_cooked_uncooked_category, shelves_shelf1_5:shelves_shelf_15ormore, level_1:level_3, 
            food_cat_chocolate_and_sugar_confectionery_energy_bars_sweet_topping_and_desserts:food_cat_sauces_dips_other_seasonings_and_dressings,
            food_cat5_healthy:food_cat5_unhealthy, 
            informas_cat_breads_rice_and_rice_products_without_added_fat_sugar_or_salt_noodles:informas_cat_baby_and_toddler_milk_formulae,
            nova_sales_processed_culinary_ingredient:nova_sales_ultraprocessed_foods, 
  ...
  mutate(
         across(c(q3p5a_small_a4_but_1_3m_x_1_9m:q3p10_combined, q5p1a:q5p1a_96, q5p2a:q5p2a_96, q4p1_4us:q4p1_asis,
                  q4p2a_whole_grain_cereals:q4p2a_infant_formula, q4p2b1_1:q4p2b1_300
                  ), ~ifelse(advert_yes == "No", "No Advert at location", .x)),
         across(c(q3p5a_small_a4_but_1_3m_x_1_9m:q3p10_combined, q5p1a:q5p1a_96, q5p2a:q5p2a_96, q4p1_4us:q4p1_asis,
                  q4p2a_whole_grain_cereals:q4p2a_infant_formula, q4p2b1_1:q4p2b1_300
                  ), ~factor(.x, levels = c("Yes", "No", "No Advert at location" )))),
  by = c("key" = "key"))%>%
  mutate_if(is.factor, ~fct_drop(.x #, only = c("")
            ))%>% #drop unused factor levels
  labelled::set_variable_labels(!!!supermarket_store_labels[names(supermarket_store_labels) %in% names(.)])%>% #labeling variables from data dictionary
  labelled::set_variable_labels(!!!supermarket_store_final_set1_2_3_labels[names(supermarket_store_final_set1_2_3_labels) %in% names(.)])%>% #labeling variables from data dictionary
  dplyr::select(county:id, location_entrance:location_checkout_edge, locationexists, food, advert_yes, q3p5a_small_a4_but_1_3m_x_1_9m:q3p10_combined,
                q5p1a:q5p1a_96, q5p2a:q5p2a_96,q7p2a_whole_grain_cereals:q7p2a_infant_formula, q7p3_1:q7p3_300, q7p5_cooked:q7p5_both,
                q7p5_not_with_cooked_uncooked_category, shelves_shelf1_5:shelves_shelf_15ormore, level_1:level_3, q4p1_4us:q4p1_asis,
                q4p2a_whole_grain_cereals:q4p2a_infant_formula, q4p2b1_1:q4p2b1_300, 
            food_cat_chocolate_and_sugar_confectionery_energy_bars_sweet_topping_and_desserts:food_cat_sauces_dips_other_seasonings_and_dressings,
...
# make dataset with variables for descriptive and inferential statistics
  ...
  dplyr::select(
    ...
    #Adverts at store
    advert_yes, q3p5a_small_a4_but_1_3m_x_1_9m:q3p10_combined, q5p1a_0:q5p1a_96, q5p2a_1:q5p2a_96,
    ...
    q7p5_cooked:q7p5_both, q7p5_not_with_cooked_uncooked_category,
    ...
    q4p1_4us:q4p1_asis, q4p2a_whole_grain_cereals:q4p2a_infant_formula, q4p2b1_1:q4p2b1_300,
  ...
  dplyr::select(-c(q7p3_14, q7p3_25, q7p3_30, q7p3_43, q7p3_49, q7p3_60, q7p3_78, q7p3_87, q7p3_94, q7p3_101, q7p3_115, q7p3_142, q7p3_146, q7p3_149,
                   ...
                   q4p2b1_198, q4p2b1_212, q4p2b1_219, q4p2b1_234, q4p2b1_239, q4p2b1_248, q4p2b1_252, q4p2b1_256, q4p2b1_265, q4p2b1_283,
                   q4p2b1_291, q4p2b1_300, q5p1a_96, q5p2a_96, food_cat_other_dairy_based_desserts,
...
* Data was transformed to `r ncol(supermarket_store_final_raw)` variables and `r nrow(supermarket_store_final_raw)` observations and then summarised
over unique rows (key) to form `r ncol(supermarket_store_final_clean)` variables and `r nrow(supermarket_store_final_clean)` observations.

```{r data description}

  tbl_summary(supermarket_store_final_analysis%>%dplyr::select(pilotoractual:consent), 
                      type = list(
...
Chi-square test and Fisher's exact test where appropriate were used for categorical data to analyze difference in proportion of all selected variables as a function of county, store size ad adverts at location(Yes/No).

For continous data, Wilcoxon rank sum test/Kruskal-Wallis rank sum test where appropriate was used to examine the comparison of county, store size
and adverts at location(Yes/No) with continous variables.
...
supermarket_store_final_analysis%>%
  ...
  labs(x= "food/drink product name sold", y="No. of stores", fill="", caption = "N=114")+
  coord_flip()+
...
supermarket_store_final_analysis%>%
  ...
  supermarket_store_theme+
  geom_hline(yintercept = 0, color = "black")+
  labs(x= "food/drink product name advertised", y="No. of stores", fill="", caption = "N=114")+
  coord_flip()+
  guides(fill= guide_legend(reverse = TRUE))
...
```{r final variables by county}
  ...
  tbl_summary(supermarket_store_final_analysis%>%dplyr::select(-c(pilotoractual:consent, q7p3_1:q7p3_299, q4p2b1_1:q4p2b1_299)),
              by = county,
                      type = list(
                        c(checkouts) ~ "continuous2",
                        all_dichotomous() ~ "categorical"
