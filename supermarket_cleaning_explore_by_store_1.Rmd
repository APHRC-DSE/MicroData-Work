...
supermarket_store_final_raw <- bind_cols(supermarket_store_final, supermarket_store_final_set1%>%dplyr::select(-key),
  ...
  dplyr::select(county:interviewdate, New_StoreLength1:id)%>%
  distinct(key, .keep_all = TRUE)%>%
    left_join(
  supermarket_store_final_raw%>%
    group_by(key, locationexists, food, Supermarket_Sizea, SESa)%>%
  ...
  kable(as.data.frame(labelled::generate_dictionary(supermarket_store_final_raw)))
