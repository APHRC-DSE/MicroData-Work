...
supermarket_store_final_set2 <- supermarket_store_final%>%
  ...
  dplyr::select(-c(setofrp_7, q3p7a, q3p9, q5p1b, q5p2b, setofrp2, parent_key, oid, q7p2b,q7p3, q7p3b,
  ...
  fastDummies::dummy_cols(select_columns = c("location", "advert", "q3p1", "q3p5a", "q3p7", "q3p8", "q3p10", "q7p2a", "q7p5", "shelves"),
  ...
  summarise(across(c(location_entrance:q3p10_combined, q5p1a:q5p2a_96, q7p2a_whole_grain_cereals:q7p2a_infant_formula, q7p3_1:q7p3_300,
                     q7p5_cooked:q7p5_both, shelves_shelf1_5:shelves_shelf_15ormore, level_1:level_3
                     ), ~ sum(.x, na.rm = TRUE)), .groups = "drop")%>%
  mutate(across(c(location_entrance:q3p10_combined, q5p1a:q5p2a_96, q7p2a_whole_grain_cereals:q7p2a_infant_formula, q7p3_1:q7p3_300,
                  q7p5_cooked:q7p5_both, shelves_shelf1_5:shelves_shelf_15ormore, level_1:level_3), ~ifelse(.x == 0, "No",
                                 ifelse(.x > 0 , "Yes", .x))),
         across(c(location_entrance:q3p10_combined, q5p1a:q5p2a_96, q7p2a_whole_grain_cereals:q7p2a_infant_formula, q7p3_1:q7p3_300,
                  q7p5_cooked:q7p5_both, shelves_shelf1_5:shelves_shelf_15ormore, level_1:level_3), ~factor(.x, levels = c("Yes", "No"))))%>%
  labelled::set_variable_labels( #labeling new variables
    ...
    advert_yes = "Do we have adverts at location",
    q3p1_1 = "Q3.1: Advertisment Number - 1",
    q3p1_2 = "Q3.1: Advertisment Number - 2",
    q3p1_3 = "Q3.1: Advertisment Number - 3",
    q3p1_4 = "Q3.1: Advertisment Number - 4",
    q3p1_5 = "Q3.1: Advertisment Number - 5",
    q3p1_6 = "Q3.1: Advertisment Number - 6",
    q3p1_7 = "Q3.1: Advertisment Number - 7",
    q3p1_8 = "Q3.1: Advertisment Number - 8",
    q3p1_9 = "Q3.1: Advertisment Number - 9",
    q3p1_10 = "Q3.1: Advertisment Number - 10",
    q3p1_11 = "Q3.1: Advertisment Number - 11",
    q3p1_13 = "Q3.1: Advertisment Number - 13",
...
supermarket_store_final_set2_labels <- as.data.frame(labelled::generate_dictionary(supermarket_store_final_set2))%>%
  select(variable, label)%>%
  deframe()
...
supermarket_store_final_clean <- purrr::reduce(list(supermarket_store_final_set1, supermarket_store_final_set2,
  ...
  mutate(across(c(q3p1_1:q5p2a_96), ~as.character(.x)),
         across(c(q3p1_1:q5p2a_96), ~ifelse(advert_yes == "No", "No Advert at location", .x)),
         across(c(q3p1_1:q5p2a_96), ~factor(.x, levels = c("Yes", "No", "No Advert at location" ))))%>%
  mutate_if(is.factor, ~fct_drop(.x #, only = c("")
            ))%>% #drop unused factor levels
  labelled::set_variable_labels(!!!supermarket_store_final_set2_labels[names(supermarket_store_final_set2_labels) %in% names(.)])%>% #labeling variables from data dictionary
  dplyr::select(county:id, locationexists, food, location_entrance:q3p10_combined, q5p1a:q5p2a_96, q7p2a_whole_grain_cereals:q7p2a_infant_formula,
                q7p3_1:q7p3_300, q7p5_cooked:q7p5_both, shelves_shelf1_5:shelves_shelf_15ormore, level_1:level_3, q4p1_4us:q4p1_asis,
                q4p2b1_1:q4p2b1_300, 
            food_cat_chocolate_and_sugar_confectionery_energy_bars_sweet_topping_and_desserts:food_cat_sauces_dips_other_seasonings_and_dressings,
...
#saving as .Rdata
save(supermarket_store_final, supermarket_store_final_clean, file = "supermarket_store_final.RData")
...
# make dataset with variables for descriptive and inferential statistics
  ...
  dplyr::select(
    ...
    #Adverts at store
    advert_yes:q3p10_combined, q5p1a_0:q5p1a_12, q5p2a_1:q5p2a_96,
